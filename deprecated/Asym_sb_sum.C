...
// -- single helicity asymmetry for blue beam A_L^B
//    (see staszak thesis, eq. 7.13; polarization weighted as in A_LL)


void Asym_sb_sum(const char * filter_type="all",Int_t filter_low=0, Int_t filter_high=0)
{
  ...
  */
  ...
  // phi_dist_num:  spinbit 3 & 2, weight P;    spinbit 1 & 0, weight R*P
  // phi_dist_den:  spinbit 3 & 2, weight P^2;  spinbit 1 & 0, weight R*P^2
      ...
      for(Int_t p=0; p<pt_bins; p++)
      {
        for(Int_t e=0; e<en_bins; e++)
        {
          ...
          for(Int_t r=0; r<NRUNS; r++)
          {
            ...
            rellum = RD->Rellum(runnum,2,"zdc"); // R2 for blue single spin asym
            //rellum=1;
            //rellum=1.001;
      ...
      for(Int_t e=0; e<en_bins; e++)
      {
        ...
        */
        ...
        sprintf(asym_t[g][p][e],
          "A_{L}^{B} vs. %s :: #eta#in[%.2f,%.2f), p_{T}#in[%.2f,%.2f), E#in[%.2f,%.2f) (runsum)",
        ...
        dist_same_num[g][p][e]->Add(phi_dist_num[3][g][p][e],phi_dist_num[2][g][p][e],1.0,1.0);
        dist_diff_num[g][p][e]->Add(phi_dist_num[1][g][p][e],phi_dist_num[0][g][p][e],1.0,1.0);
        dist_same_den[g][p][e]->Add(phi_dist_den[3][g][p][e],phi_dist_den[2][g][p][e],1.0,1.0);
        dist_diff_den[g][p][e]->Add(phi_dist_den[1][g][p][e],phi_dist_den[0][g][p][e],1.0,1.0);
  ...
  if(eta_bins==1)
  {
    ...
    for(Int_t p=0; p<pt_bins; p++)
    {
      ...
      sprintf(en_dep_t[p],"A_{L}^{B} vs. E_{#gamma#gamma} for p_{T}#in[%.2f,%.2f)",pt_div[p],pt_div[p+1]);
    ...
    for(Int_t e=0; e<en_bins; e++)
    {
      ...
      sprintf(pt_dep_t[e],"A_{L}^{B} vs. p_{T} for E_{#gamma#gamma}#in[%.2f,%.2f)",en_div[e],en_div[e+1]);
  ...
  for(Int_t p=0; p<pt_bins; p++)
  {
    en_dep[p]->GetYaxis()->SetTitle("A_{L}^{B}");
  ...
  for(Int_t e=0; e<en_bins; e++)
  {
    pt_dep[e]->GetYaxis()->SetTitle("A_{L}^{B}");
