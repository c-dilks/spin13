...
// -- double helicity asymmetry A_LL
//    (see staszak thesis, eq. 7.5)

void Asym_sum(const char * filter_type="all",Int_t filter_low=0, Int_t filter_high=0)
{
  ...
  */
  ...
  // phi_dist_num:  spinbit 0 & 3, weight P;    spinbit 1 & 2, weight R*P
  // phi_dist_den:  spinbit 0 & 3, weight P^2;  spinbit 1 & 2, weight R*P^2
    ...
    for(Int_t p=0; p<pt_bins; p++)
    {
      for(Int_t e=0; e<en_bins; e++)
      {
        ...
        */
        ...
        dist_same_num[g][p][e]->Add(phi_dist_num[0][g][p][e],phi_dist_num[3][g][p][e],1.0,1.0);
        dist_diff_num[g][p][e]->Add(phi_dist_num[1][g][p][e],phi_dist_num[2][g][p][e],1.0,1.0);
        dist_same_den[g][p][e]->Add(phi_dist_den[0][g][p][e],phi_dist_den[3][g][p][e],1.0,1.0);
        dist_diff_den[g][p][e]->Add(phi_dist_den[1][g][p][e],phi_dist_den[2][g][p][e],1.0,1.0);
  ...
  /*
  // plot pt or en dependence -- OLD VERSION
  ...
  TGraphErrors * en_dep[pt_bins];
  TGraphErrors * pt_dep[en_bins];
  ...
  */
  ...
  if(eta_bins==1)
  {
    ...
    for(Int_t p=0; p<pt_bins; p++)
    {
      ...
      sprintf(en_dep_t[p],"A_{LL} vs. E_{#gamma#gamma} for p_{T}#in[%.2f,%.2f)",pt_div[p],pt_div[p+1]);
    ...
    for(Int_t e=0; e<en_bins; e++)
    {
      ...
      sprintf(pt_dep_t[e],"A_{LL} vs. p_{T} for E_{#gamma#gamma}#in[%.2f,%.2f)",en_div[e],en_div[e+1]);
  ...
  for(Int_t p=0; p<pt_bins; p++)
  {
    en_dep[p]->GetYaxis()->SetTitle("A_{LL}");
  ...
  for(Int_t e=0; e<en_bins; e++)
  {
    pt_dep[e]->GetYaxis()->SetTitle("A_{LL}");
  ...
  // write phi dists
  printf("writing spin_sum.root...\n");
  TFile * outfile = new TFile("spin_sum.root","RECREATE");
