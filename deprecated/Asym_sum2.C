...
// using method in 2009 inclusive jet A_LL paper
// -- sum over phi distributions; multiply numerator terms by P_B * P_Y and 
//    denominator terms by (P_B * P_Y)^2
// -- error bars are handled by root (assume sqrt(N) for yield uncertainties)
...
void Asym_sum2()
{
  ...
  TObjArray * phi_dist_arr_mP[4][eta_bins][pt_bins][en_bins];
  TObjArray * phi_dist_arr_mP2[4][eta_bins][pt_bins][en_bins];
  char phi_dist_arr_n[4][eta_bins][pt_bins][en_bins][64];
  char phi_dist_arr_mP_n[4][eta_bins][pt_bins][en_bins][64];
  char phi_dist_arr_mP2_n[4][eta_bins][pt_bins][en_bins][64];
  Int_t NRUNS_tmp;
  for(Int_t s=0; s<4; s++)
  {
    for(Int_t g=0; g<eta_bins; g++)
    {
      for(Int_t p=0; p<pt_bins; p++)
      {
        for(Int_t e=0; e<en_bins; e++)
        {
          ...
          sprintf(phi_dist_arr_mP_n[s][g][p][e],"phi_dist_mP_s%d_g%d_p%d_e%d",s,g,p,e);
          sprintf(phi_dist_arr_mP2_n[s][g][p][e],"phi_dist_mP2_s%d_g%d_p%d_e%d",s,g,p,e);
          phi_dist_arr[s][g][p][e] = (TObjArray*) infile->Get(phi_dist_arr_n[s][g][p][e]);
          phi_dist_arr_mP[s][g][p][e] = (TObjArray*) infile->Get(phi_dist_arr_mP_n[s][g][p][e]);
          phi_dist_arr_mP2[s][g][p][e] = (TObjArray*) infile->Get(phi_dist_arr_mP2_n[s][g][p][e]);
          ...
          else
          {
            if(phi_dist_arr[s][g][p][e]->GetEntries() != NRUNS_tmp ||
               phi_dist_arr_mP[s][g][p][e]->GetEntries() != NRUNS_tmp ||
               phi_dist_arr_mP2[s][g][p][e]->GetEntries() != NRUNS_tmp)
            {
  ...
  // build summed histograms and weight spinbits 1 & 2 with rellum
  TH1D * phi_dist[4][eta_bins][pt_bins][en_bins];
  TH1D * phi_dist_mP[4][eta_bins][pt_bins][en_bins];
  TH1D * phi_dist_mP2[4][eta_bins][pt_bins][en_bins];
  char phi_dist_n[4][eta_bins][pt_bins][en_bins][128];
  char phi_dist_mP_n[4][eta_bins][pt_bins][en_bins][128];
  char phi_dist_mP2_n[4][eta_bins][pt_bins][en_bins][128];
  Int_t runnum;
  Float_t R3,weight;
  for(Int_t s=0; s<4; s++)
  {
    for(Int_t g=0; g<eta_bins; g++)
    {
      for(Int_t p=0; p<pt_bins; p++)
      {
        for(Int_t e=0; e<en_bins; e++)
        {
          ...
          sprintf(phi_dist_mP_n[s][g][p][e],"phi_mP_s%d_g%d_p%d_e%d",s,g,p,e);
          sprintf(phi_dist_mP2_n[s][g][p][e],"phi_mP2_s%d_g%d_p%d_e%d",s,g,p,e);
          phi_dist[s][g][p][e] = new TH1D(phi_dist_n[s][g][p][e],phi_dist_n[s][g][p][e],phi_bins,-1*pi,pi);
          phi_dist_mP[s][g][p][e] = new TH1D(phi_dist_mP_n[s][g][p][e],phi_dist_mP_n[s][g][p][e],phi_bins,-1*pi,pi);
          phi_dist_mP2[s][g][p][e] = new TH1D(phi_dist_mP2_n[s][g][p][e],phi_dist_mP2_n[s][g][p][e],phi_bins,-1*pi,pi);
          ...
          for(Int_t r=0; r<NRUNS; r++)
          {
            ...
            if(s==1 || s==2) weight = R3;
            else weight = 1.0;
            phi_dist[s][g][p][e]->Add((TH1D*)(phi_dist_arr[s][g][p][e]->At(r)),weight);
            phi_dist_mP[s][g][p][e]->Add((TH1D*)(phi_dist_arr_mP[s][g][p][e]->At(r)),weight);
            phi_dist_mP2[s][g][p][e]->Add((TH1D*)(phi_dist_arr_mP2[s][g][p][e]->At(r)),weight);
          };
        };
      };
    };
  ...
  TH1D * dist_same_mP[eta_bins][pt_bins][en_bins]; // S_mP = P * (N++ + N--)
  TH1D * dist_diff_mP[eta_bins][pt_bins][en_bins]; // D_mP = P * ( R*(N+- + N-+) )
  TH1D * dist_same_mP2[eta_bins][pt_bins][en_bins]; // S_mP2 = P^2 * (N++ + N--)
  TH1D * dist_diff_mP2[eta_bins][pt_bins][en_bins]; // D_mP2 = P^2 * ( R*(N+- + N-+) )
  TH1D * numer[eta_bins][pt_bins][en_bins]; // S_mP - D_mP
  TH1D * denom[eta_bins][pt_bins][en_bins]; // S_mP2 + D_mP2
  TH1D * asym[eta_bins][pt_bins][en_bins]; // numer / denom
  Int_t asym_pts[eta_bins][pt_bins][en_bins]; // number of points in asym
  char dist_same_n[eta_bins][pt_bins][en_bins][128];
  char dist_diff_n[eta_bins][pt_bins][en_bins][128];
  char dist_same_mP_n[eta_bins][pt_bins][en_bins][128];
  char dist_diff_mP_n[eta_bins][pt_bins][en_bins][128];
  char dist_same_mP2_n[eta_bins][pt_bins][en_bins][128];
  char dist_diff_mP2_n[eta_bins][pt_bins][en_bins][128];
    ...
    for(Int_t p=0; p<pt_bins; p++)
    {
      for(Int_t e=0; e<en_bins; e++)
      {
        ...
        */
        ...
        sprintf(dist_same_mP_n[g][p][e],"dist_same_mP_g%d_p%d_e%d",g,p,e);
        sprintf(dist_same_mP2_n[g][p][e],"dist_same_mP2_g%d_p%d_e%d",g,p,e);
        sprintf(dist_diff_mP_n[g][p][e],"dist_diff_mP_g%d_p%d_e%d",g,p,e);
        sprintf(dist_diff_mP2_n[g][p][e],"dist_diff_mP2_g%d_p%d_e%d",g,p,e);
        ...
        sprintf(asym_t[g][p][e],
          "A_{LL} vs. %s :: #eta#in[%.2f,%.2f), p_{T}#in[%.2f,%.2f), E#in[%.2f,%.2f) (runsum)",
        ...
        dist_same_mP[g][p][e] = new TH1D(dist_same_mP_n[g][p][e],
            dist_same_mP_n[g][p][e],phi_bins,phi_low,phi_high);
        dist_same_mP2[g][p][e] = new TH1D(dist_same_mP2_n[g][p][e],
            dist_same_mP2_n[g][p][e],phi_bins,phi_low,phi_high);
        dist_diff_mP[g][p][e] = new TH1D(dist_diff_mP_n[g][p][e],
            dist_diff_mP_n[g][p][e],phi_bins,phi_low,phi_high);
        dist_diff_mP2[g][p][e] = new TH1D(dist_diff_mP2_n[g][p][e],
            dist_diff_mP2_n[g][p][e],phi_bins,phi_low,phi_high);
        ...
        dist_same[g][p][e]->Add(phi_dist[0][g][p][e],phi_dist[3][g][p][e],1.0,1.0);
        dist_diff[g][p][e]->Add(phi_dist[1][g][p][e],phi_dist[2][g][p][e],1.0,1.0);

        dist_same_mP[g][p][e]->Add(phi_dist_mP[0][g][p][e],phi_dist_mP[3][g][p][e],1.0,1.0);
        dist_diff_mP[g][p][e]->Add(phi_dist_mP[1][g][p][e],phi_dist_mP[2][g][p][e],1.0,1.0);

        dist_same_mP2[g][p][e]->Add(phi_dist_mP2[0][g][p][e],phi_dist_mP2[3][g][p][e],1.0,1.0);
        dist_diff_mP2[g][p][e]->Add(phi_dist_mP2[1][g][p][e],phi_dist_mP2[2][g][p][e],1.0,1.0);
        
        numer[g][p][e]->Add(dist_same_mP[g][p][e],dist_diff_mP[g][p][e],1.0,-1.0);
        denom[g][p][e]->Add(dist_same_mP2[g][p][e],dist_diff_mP2[g][p][e],1.0,1.0);
  ...
  // write phi dists
  printf("writing spin_sum2.root...\n");
  TFile * outfile = new TFile("spin_sum2.root","RECREATE");
  for(Int_t g=0; g<eta_bins; g++)
  {
    for(Int_t p=0; p<pt_bins; p++)
    {
      for(Int_t e=0; e<en_bins; e++)
      {
        for(Int_t s=0; s<4; s++)
        {
          ...
          phi_dist_mP[s][g][p][e]->Write(phi_dist_mP_n[s][g][p][e]);
          phi_dist_mP2[s][g][p][e]->Write(phi_dist_mP2_n[s][g][p][e]);
        };
      };
    };
  };
