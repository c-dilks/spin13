...
void DrawAN2(const char * filename="RedOutputset107ha.root", Int_t nbins_E=15, Int_t nbins_Pt=15)
{
  ...
  for(Int_t n_E=0; n_E<Ebins; n_E++)
  {
    for(Int_t n_Pt=0; n_Pt<Ptbins; n_Pt++)
    {
      ...
      {
        blue_an[n_E][n_Pt] =
         new TH1F((*(cos_phi_up[n_E][n_Pt])-*(cos_phi_dn[n_E][n_Pt]))/(*(cos_phi_up[n_E][n_Pt])+*(cos_phi_dn[n_E][n_Pt])));
      ...
      else blue_an[n_E][n_Pt] = new TH1F(blue_an_n[n_E][n_Pt],blue_an_t[n_E][n_Pt],COS_BINS,-1,1);
    };
  };
