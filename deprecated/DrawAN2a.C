// draws cos(phi) distributions for various E:Pt bins and computes AN
// - The AN vs. cos(phi) plot is computed manually rather than by making a new histogram
//   from the old ones using mathematical operations (as done in DrawAN2.C)

void DrawAN2a(const char * filename="RedOutputset107ha.root", Int_t nbins_E=15, Int_t nbins_Pt=15)
{
  ...
  TH1F * cos_phi_up[Ebins][Ptbins];
  TH1F * cos_phi_dn[Ebins][Ptbins];
  ...
  for(Int_t n_E=0; n_E<Ebins; n_E++)
  {
    for(Int_t n_Pt=0; n_Pt<Ptbins; n_Pt++)
    {
      ...
      sprintf(cos_phi_dn_t[n_E][n_Pt],
              ...
              E_arr[n_E],E_arr[n_E]+E_div,Pt_arr[n_Pt],Pt_arr[n_Pt]+Pt_div);
      cos_phi_up[n_E][n_Pt] = new TH1F(cos_phi_up_n[n_E][n_Pt],cos_phi_up_t[n_E][n_Pt],COS_BINS,-1,1);
      cos_phi_dn[n_E][n_Pt] = new TH1F(cos_phi_dn_n[n_E][n_Pt],cos_phi_dn_t[n_E][n_Pt],COS_BINS,-1,1);
  ...
  for(Int_t i=0; i<ent; i++)
  {
    ...
    if(spin==0 || spin==1) cos_phi_dn[n_E_set][n_Pt_set]->Fill(cos(Phi));
    else if(spin==2 || spin==3) cos_phi_up[n_E_set][n_Pt_set]->Fill(cos(Phi));
  };
  printf("end of str read loop\n");

  
  // calculate AN
  TH1F * blue_an[Ebins][Ptbins];
  TH2F * blue_an_2d_pos = 
  ...
  Float_t bc_up,bc_dn,asym;
  for(Int_t n_E=0; n_E<Ebins; n_E++)
  {
    for(Int_t n_Pt=0; n_Pt<Ptbins; n_Pt++)
    {
      /*
      full_cos_dist=1; // false if up (down) cos(phi) dist has a zero bin where down (up) doesn't
      ...
      */
      ...
      if(cos_phi_up[n_E][n_Pt]->GetEntries() * cos_phi_dn[n_E][n_Pt]->GetEntries())
      {
        for(Int_t bn=0; bn<=cos_phi_up[n_E][n_Pt]->GetNbinsX(); bn++)
        {
          bc_up = cos_phi_up[n_E][n_Pt]->GetBinContent(bn);
          bc_dn = cos_phi_dn[n_E][n_Pt]->GetBinContent(bn);
          // require at least one entry in bin for both up and down dists
          if(bc_up>=1 && bc_dn>=1)
          {
            asym = (bc_up-bc_dn)/(bc_up+bc_dn);
            blue_an[n_E][n_Pt]->SetBinContent(bn,asym);
            /* debug */
            if(n_E==4 && n_Pt==7) printf("bc_up=%f bc_dn=%f asym=%f\n",bc_up,bc_dn,asym);
          };
        };
  ...
  for(Int_t n_E=0; n_E<Ebins; n_E++)
  {
    for(Int_t n_Pt=0; n_Pt<Ptbins; n_Pt++)
    {
      cc->Clear();
      cc->Divide(2,2);
      cc->cd(1); cos_phi_up[n_E][n_Pt]->Draw("e");
      cc->cd(2); cos_phi_dn[n_E][n_Pt]->Draw("e");
