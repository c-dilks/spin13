...
//
// -- KINEMATIC CUTS ARE VERY LOOSE; THIS IS "OPEN" ANALYSIS!

void PhiDists_open(const char * filename="RedOutputset120ha.root",Int_t phi_bins0=16,
              Int_t eta_bins0=1, Int_t pt_bins0=3, Int_t en_bins0=3)
{
  ...
  */
  ...
  char trig_cut[32]; sprintf(trig_cut,"1"); // other cuts
  char z_cut[16]; sprintf(z_cut,"1");
  char mass_cut[32]; sprintf(mass_cut,"1");
    ...
    for(Int_t g=0; g<eta_bins; g++)
    {
      for(Int_t p=0; p<pt_bins; p++)
      {
        for(Int_t e=0; e<en_bins; e++)
        {
          ...
          tree->Project(phi_dist_n[s][g][p][e],"Phi",kinematic_cut[s][g][p][e]);
