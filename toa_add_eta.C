// TObjArray Adder
// 
// combines and organises eta distributions from TObjArrays
// of etaset files
//
// -- reads rtree.root
...
{
  gROOT->Reset();
  // build array of etaset/*.root TFile pointers
  const Int_t MAX_NUM_FILES=200;
  TFile * eta_file[MAX_NUM_FILES]; 
  Int_t eta_file_cnt=0;
  gROOT->ProcessLine(".! ls etaset/eta*.root > toa_files.txt");
  ...
  else
  {
    ...
    {
      fgets(filename[eta_file_cnt],filename_buffer,toa_files);

      // fgets reads in "returns"; this hack gets rid of them
      sscanf(filename[eta_file_cnt],"%s",filename[eta_file_cnt]);

      if(strcmp(filename[eta_file_cnt],""))
      {
        printf("%d: %s\n",eta_file_cnt,filename[eta_file_cnt]);
        eta_file[eta_file_cnt] = new TFile(filename[eta_file_cnt],"READ");
        eta_file_cnt++;
      };
    };
  };
  const Int_t NFILES=eta_file_cnt;
  ...
  // eta_dist [spin] [phi] [pt] [energy] [run index - 1]
  TH1D * eta_dist_sph[4][phi_bins][pt_bins][en_bins][rtree_ent];
  TH1D * eta_dist_pi0[4][phi_bins][pt_bins][en_bins][rtree_ent];
  TH1D * eta_dist_thr[4][phi_bins][pt_bins][en_bins][rtree_ent];
  
  // infile_arr [spin] [phi] [pt] [energy] [eta file]
  TObjArray * infile_sph_arr[4][phi_bins][pt_bins][en_bins][NFILES];
  TObjArray * infile_pi0_arr[4][phi_bins][pt_bins][en_bins][NFILES];
  TObjArray * infile_thr_arr[4][phi_bins][pt_bins][en_bins][NFILES];
  char infile_sph_arr_n[4][phi_bins][pt_bins][en_bins][200];
  char infile_pi0_arr_n[4][phi_bins][pt_bins][en_bins][200];
  char infile_thr_arr_n[4][phi_bins][pt_bins][en_bins][200];
  ...
  {
    eta_file[f]->cd(); // focus on next TFile
    for(Int_t s=0; s<4; s++)
    {
      for(Int_t g=0; g<phi_bins; g++)
      {
        for(Int_t p=0; p<pt_bins; p++)
        {
          for(Int_t e=0; e<en_bins; e++)
          {
            ...
            if(f==0)
            {
              sprintf(infile_sph_arr_n[s][g][p][e],"/sph/eta_dist_sph_s%d_g%d_p%d_e%d",s,g,p,e);
              sprintf(infile_pi0_arr_n[s][g][p][e],"/pi0/eta_dist_pi0_s%d_g%d_p%d_e%d",s,g,p,e);
              sprintf(infile_thr_arr_n[s][g][p][e],"/thr/eta_dist_thr_s%d_g%d_p%d_e%d",s,g,p,e);
            };
            // read TObjArrays
            infile_sph_arr[s][g][p][e][f] = (TObjArray*)eta_file[f]->Get(infile_sph_arr_n[s][g][p][e]);
            infile_pi0_arr[s][g][p][e][f] = (TObjArray*)eta_file[f]->Get(infile_pi0_arr_n[s][g][p][e]);
            infile_thr_arr[s][g][p][e][f] = (TObjArray*)eta_file[f]->Get(infile_thr_arr_n[s][g][p][e]);
            ...
            for(Int_t o=0; o<infile_sph_arr[s][g][p][e][f]->GetEntries(); o++)
            {
              ...
              sscanf(infile_sph_arr[s][g][p][e][f]->At(o)->GetName(),
                "eta_sph_s%*d_g%*d_p%*d_e%*d_r%d",&inrun);
              // linear hash --> typcast eta_dist's
              for(Int_t h=0; h<rtree_ent; h++)
              {
                if(inrun == runnum_arr[h])
                {
                  eta_dist_sph[s][g][p][e][h] = (TH1D*) infile_sph_arr[s][g][p][e][f]->At(o);
                  filter_sph[h]=true;
                };
              };
            };
            ...
            for(Int_t o=0; o<infile_pi0_arr[s][g][p][e][f]->GetEntries(); o++)
            {
              ...
              sscanf(infile_pi0_arr[s][g][p][e][f]->At(o)->GetName(),
                "eta_pi0_s%*d_g%*d_p%*d_e%*d_r%d",&inrun);
              // linear hash --> typcast eta_dist's
              for(Int_t h=0; h<rtree_ent; h++)
              {
                if(inrun == runnum_arr[h])
                {
                  eta_dist_pi0[s][g][p][e][h] = (TH1D*) infile_pi0_arr[s][g][p][e][f]->At(o);
                  filter_pi0[h]=true;
                };
              };
            };
            ...
            for(Int_t o=0; o<infile_thr_arr[s][g][p][e][f]->GetEntries(); o++)
            {
              ...
              sscanf(infile_thr_arr[s][g][p][e][f]->At(o)->GetName(),
                "eta_thr_s%*d_g%*d_p%*d_e%*d_r%d",&inrun);
              // linear hash --> typcast eta_dist's
              for(Int_t h=0; h<rtree_ent; h++)
              {
                if(inrun == runnum_arr[h])
                {
                  eta_dist_thr[s][g][p][e][h] = (TH1D*) infile_thr_arr[s][g][p][e][f]->At(o);
                  filter_thr[h]=true;
                };
              };
            };
  ...
  TFile * outfile = new TFile("etaset/all.root","RECREATE");
  outfile->cd();
  outfile->mkdir("sph");
  outfile->mkdir("pi0");
  outfile->mkdir("thr");
  TObjArray * combined_sph_array[4][phi_bins][pt_bins][en_bins];
  TObjArray * combined_pi0_array[4][phi_bins][pt_bins][en_bins];
  TObjArray * combined_thr_array[4][phi_bins][pt_bins][en_bins];
  char combined_sph_array_n[4][phi_bins][pt_bins][en_bins][200];
  char combined_pi0_array_n[4][phi_bins][pt_bins][en_bins][200];
  char combined_thr_array_n[4][phi_bins][pt_bins][en_bins][200];
    ...
    for(Int_t g=0; g<phi_bins; g++)
    {
      for(Int_t p=0; p<pt_bins; p++)
      {
        for(Int_t e=0; e<en_bins; e++)
        {
          ...
          sprintf(combined_sph_array_n[s][g][p][e],"eta_dist_sph_s%d_g%d_p%d_e%d",s,g,p,e);
          sprintf(combined_pi0_array_n[s][g][p][e],"eta_dist_pi0_s%d_g%d_p%d_e%d",s,g,p,e);
          sprintf(combined_thr_array_n[s][g][p][e],"eta_dist_thr_s%d_g%d_p%d_e%d",s,g,p,e);

          for(Int_t r=0; r<rtree_ent; r++)
          {
            printf("%d.%p %d.%p %d.%p\n",filter_sph[r],(void*)eta_dist_sph[s][g][p][e][r],
                                         filter_pi0[r],(void*)eta_dist_pi0[s][g][p][e][r],
                                         filter_thr[r],(void*)eta_dist_thr[s][g][p][e][r]);
            if(eta_dist_sph[s][g][p][e][r]!=NULL && filter_sph[r]==1)
              combined_sph_array[s][g][p][e]->AddLast(eta_dist_sph[s][g][p][e][r]);
            if(eta_dist_pi0[s][g][p][e][r]!=NULL && filter_pi0[r]==1)
              combined_pi0_array[s][g][p][e]->AddLast(eta_dist_pi0[s][g][p][e][r]);
            if(eta_dist_thr[s][g][p][e][r]!=NULL && filter_thr[r]==1)
              combined_thr_array[s][g][p][e]->AddLast(eta_dist_thr[s][g][p][e][r]);
          };
  ...
  printf("etaset/all.root written\n");
}
